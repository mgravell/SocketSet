...
internal static unsafe partial class NativeOpenSsl
{
    ...
    // renegotiation); it does NOT affect TLS 1.3 KeyUpdate, which stays allowed. Set on the SERVER context
    // only — a client that set it would refuse legitimate server-initiated renegotiation (see OpenSslTlsProvider).
