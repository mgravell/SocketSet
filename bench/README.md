...
| **`resp-benchmark`** (the RESP generator) | Not a rig here — the GENERATOR the Windows Garnet work uses, recorded because picking it was a measurement decision. **`dotnet tool install -g RESPite.Benchmark --version 3.1.13`**, and pin that version: an instrument must be fixed and identified, not current. Takes redis-benchmark's arguments (`-h -p -c -n -d -P -t -q -l`) plus `+m` and `--batch`/`--queue`. **Its classic SE.Redis leg opens exactly ONE connection, whatever `-c` says** — verified 2026-08-07 by sampling `Get-NetTCPConnection` on the server port six times during a `-c 50` run: 1 every time. `-c` is in-flight CONCURRENCY pipelined onto that one connection, not connection count, and `+m` changes nothing (the readme's "each client has a separate connection" does not describe this leg). So **every number it produces is single-connection, single-shard**, and a transport A/B needs many connections from somewhere else — a client that opens N, or N processes summed. **Sample the connection count during the run**, not after: a fast run finishes before the sample lands and reports 0. The rejected alternative is on the box and worth knowing about: `redis-benchmark.exe` 3.0.503 (`StackExchange.Redis/tests/RedisConfigs/3.0.503`, the dead MSOpenTech Windows fork) is single-threaded and ceilings at ~92k, where three structurally different transports measured inside **1.3%** — a generator bound reading as parity. |
