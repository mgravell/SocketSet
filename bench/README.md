...
| **`Verify-GarnetDemo.ps1`** | *"Does GarnetDemo actually SERVE on this OS, on every backend it claims?"* Added 2026-08-07 with the Windows hosting work. 16 cells (iocp/rio/managed/default/stock × plaintext/TLS, plus the refusals), ~1 min. The discriminating assertion is a **real RESP round-trip** — PING must return `+PONG` — not a banner and not a process that stays up, because a server that binds and never answers looks identical to a working one. The banner is asserted separately (a cell that asked for RIO and silently got IOCP would round-trip perfectly). Cross-platform in intent: it picks this OS's backends and asserts the refusal of the other OS's. **Its own first cut was a broken rig** — it sent plaintext at the TLS port and reported a read timeout, which reads exactly like a broken server. |
| **`resp-benchmark`** (the RESP generator) | Not a rig here — the GENERATOR the Garnet work uses, recorded because picking it was a measurement decision. **`dotnet tool install -g RESPite.Benchmark --version 3.1.14`**, and pin that version: an instrument must be fixed and identified, not current. Takes redis-benchmark's arguments (`-h -p -c -n -d -P -t -q -l`) plus `+m` and `--batch`/`--queue`. **The 3.1.13 pin's classic SE.Redis leg opened exactly ONE connection, whatever `-c` said** (verified 2026-08-07 on Windows via `Get-NetTCPConnection`, six samples during a `-c 50` run: 1 every time) — `-c` was in-flight concurrency pipelined onto one connection, so every 3.1.13 number is single-connection, single-shard. **3.1.14 fixes it, and the fix was verified 2026-08-10 on Linux by the discriminating PAIR**: same box, same GarnetDemo/io_uring server, same `-c 50` command — 3.1.13 showed 1 benchmark connection in `CLIENT LIST` mid-run, 3.1.14 showed 50, and its banner now self-reports `(clients: 50, conns: 50)`, which is what `Run-GarnetAb.ps1` gates on. **Sample the connection count during the run**, not after: a fast run finishes before the sample lands and reports 0. One process still ceilings around ~270k ops/s regardless of connections (confounder 1), so multi-process fan-out remains necessary for saturation. The rejected alternative is on the box and worth knowing about: `redis-benchmark.exe` 3.0.503 (`StackExchange.Redis/tests/RedisConfigs/3.0.503`, the dead MSOpenTech Windows fork) is single-threaded and ceilings at ~92k, where three structurally different transports measured inside **1.3%** — a generator bound reading as parity. |
