...
# SmokeTest also rescales WriteBuffersPerShard and OutOfBandWriteBuffersPerShard to 4MB/N, to hold pinned
# memory roughly constant. So a bare sweep varies page size AND pool depth together. On Windows that was
# shown not to matter at 256KB (2,387.7 vs 2,387.2 MiB/s, 0.02% apart) - but that is a Windows result at
# one payload. Set FIXED_WRITE_BUFFERS to pin the pool depth across all pages and separate the two.
#
# Usage:  ./run-page-sizes.sh
#         PAGES="4096 16384 65536 262144" SIZES="262144" ./run-page-sizes.sh
#         FIXED_WRITE_BUFFERS=256 ./run-page-sizes.sh      # control for the co-variation above
...
PORT=${PORT:-5081}
FILTER=${FILTER:-}
# Empty = let --page rescale the pool (the shipped behaviour). A number pins it for every page size.
FIXED_WRITE_BUFFERS=${FIXED_WRITE_BUFFERS:-}
...
echo "size,page,backend,shards,writebufs,rep,rps,mib_s,lat_p50_us,lat_p99_us,errors,status" > "$CSV"
...
measure() { # $1=backend $2=page $3=size $4=rep
  ...
  [[ -n "$FIXED_WRITE_BUFFERS" ]] && args+=(--write-buffers "$FIXED_WRITE_BUFFERS")
  ...
  # Record the pool depth that actually resulted, so the co-variation is visible in the CSV rather than
  # having to be recomputed from the page size later.
  local wb; wb=$(sed -n 's/.*writebufs=\([0-9]*\).*/\1/p' <<<"$banner")
  ...
  if [[ -z "$rps" ]]; then
    ...
    echo "$size,$page,$backend,$SHARDS,$wb,$rep,,,,,,FAILED" >>"$CSV"; return
  fi
  ...
  echo "$size,$page,$backend,$SHARDS,$wb,$rep,${rps%.*},$mib,${p50%.*},${p99%.*},$errs,$([[ $errs -gt 0 ]] && echo ERRORS || echo ok)" >>"$CSV"
}
...
echo "  pool  : $([[ -n "$FIXED_WRITE_BUFFERS" ]] && echo "PINNED at $FIXED_WRITE_BUFFERS buffers/shard (co-variation controlled)" || echo "follows --page (4MB/page) - co-variation NOT controlled")"
...
awk -F, 'NR>1 && $6>1 && $8!="" { k=$1"|"$3"|"$2; n[k]++; v[k"_"n[k]]=$8+0 }
  END {
    ...
    for (i=1;i<=m;i++) { s=ss[i]; printf "%-10s", s
      ...
      printf "\n" }
  }' "$CSV" 2>/dev/null || awk -F, 'NR>1 && $6>1 && $8!="" {printf "%-8s p%-8s %-10s %10s MiB/s\n",$1,$2,$3,$8}' "$CSV"
